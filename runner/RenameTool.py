...
class RenameTool(QMW, RTUI):
    ...
    def _connect(self):
        ''' 统一绑定各种控件到相应函数。
            ...
            btn_TS_PrevNRun：     任务列表的“预览/执行”按钮
        '''
        ...
        self.btn_TS_PrevNRun.clicked.connect(self._taskprevnrun)
    ...
    def _taskprevnrun(self):
        ...
        if (ind := self.list_Tasks.currentRow()) != -1:
            string = f'{"*" * 100}\n'.join(
                [f'原文件名：{key}\n重命名为：{val}\n' for key, val in self.tasklist[ind].preview()[0].items()])
            prevwindow.textEdit.setText(string)
            prevwindow.show()
